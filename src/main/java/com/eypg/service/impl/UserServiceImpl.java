  ...
  public void delCollectGoods(Integer id)
  {
	...
	private User paid(final User user, final List<ProductCart> productCartList, final List<ProductJSON> successCartList,
		...
		for (final ProductCart productCart : productCartList) {
			...
			// buy - product
			final Spellbuyproduct buyProduct = buyProductService.findById(""+productCart.getProductId());
			//本期剩余可购买数
			final int remain = buyProduct.getSpellbuyPrice()-buyProduct.getSpellbuyCount();
			//本期实际购买数
			final int dealCount = Math.min(count, remain);
			if(dealCount<count){
				//超拍部分转入余额
				recharge(user,count-dealCount);
			}
			if(dealCount<=0){
				continue;
			}
			buyProduct.setSpellbuyCount(buyProduct.getSpellbuyCount()+dealCount);
			// cons-detail
			...
			//买完的话就直接开新一期
			if(buyProduct.getSpellbuyCount().intValue()==buyProduct.getSpellbuyPrice()){
				buyProductService.generateNextPeriod(buyProduct);
			}
		}// for-cart-list
		return user;
	}
		
	private final User recharge(final User user, final double money){
		double ba = user.getBalance()+money;
		user.setBalance(ba);
		return user;
	}
